                ...
                <div class="box-header with-border">
                    <div class="box-tools">
                        <div class="btn-group">
                            @isset($create_access)
                            @if ($create_access == 1)
                            <a href="{{ url(route('create-chain')) }}" class="btn btn-sm btn-primary"><i class="fa fa-plus fa-fw"></i>Create Chain</a>
                            @endif
                            @endisset
                        </div>
                    </div>
                    <h3 class="box-title">Chains List</h3>
                </div>
                <div class="box-body">
...
<script type="text/javascript">
            ...
            'action_button' : {
                ...
                'action': [
                {
                    ...
                    },
                    {
                        ...
                        "title"     : 'Change status',
                        "url"       : function(data){
                            return "chain-update-status/"+data['uuid'];
                        },
                        "icon"      : "fa fa-check",
                        "method"    : "get",
                        "condition" : function(data){
                        ...
                        "function_call" : "changechainStatus"
                    },
                    {
                        "name"      : "In-Active",
                        "type"      : "dialog", //view,dialog,modal
         ...
         * @param string method
         */
         function chainDeleteRecord(url, method) {
            var request  = {
                'url'             : url,
                'type'            : method,
                'data'            : {},
         ...
         * @param Object current object
         */
         function changechainStatus(current) {
            var is_confirm = $(current).data('is_confirm');
            var method = $(current).data('method');
            var url = $(current).data('url');
            if(is_confirm) {
         ...
         * @param string method
         */
         function updatechainStatus(url, method) {
            var request  = {
                'url'             : url,
                'type'            : method,
                'data'            : {},
